...
PATH_DATA = ROOT_DIR / "energy_insights_dashboard/data/StromErzeugung2022Stündlich.csv"
...
pipeline = pdp.PdPipeline(
    [
        ...
        pdp.ColRename({"Date": "Datum"}),
        ToDateTimeTransformer("Datum", "%Y-%m-%d"),
        ToTimeDeltaTransformer("Start"),
        pdp.df["Datum"] << pdp.df["Datum"] + pdp.df["Start"],
        pdp.ColDrop(["Start", "End"], errors="ignore"),
        pdp.SetIndex("Datum"),
...
if __name__ == "__main__":
    ...
    df_renewable_vs_rest_stacked.reset_index().to_pickle(
        ROOT_DIR / "energy_insights_dashboard/data/electricity_2022_hourly.pkl"
    )
